...
export default class GameConsoleList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getServiceData = (page, pageNumber) => {
    ...
    fetch(api(`services/page/${page}/console`)).then((res) => {
      if (res.ok) {
  ...
  addConsole() {
    this.props.history.push("/game/console/insert");
  }
            ...
            <CCard className="mb-4">
              <CCardHeader>
                <span className="d-grid gap-2 d-md-flex justify-content-between">
                  <strong>Liste des consoles</strong>
                  <CButton color={"primary"} onClick={() => this.addConsole()}>
